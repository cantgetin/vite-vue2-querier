<template>
  <div class="about">
    <h2> Отчет по количеству входящих, исходящих, исходящих с ЭЦП и проценту подписанных документов</h2>
    <hr>
    <br>
    <h3 v-if="!date.dateValueTo.length" >Выберите даты отчета</h3>
    <h3 v-if="outEcpData.length && date.dateValueTo.length" >Нажмите "Скачать отчет", для загрузки</h3>
    <br>
    <div class="container shadow p-3 mb-3 bg-white rounded" >
      
      <div class="controls">
        <b-button class="btn-back" variant="dark" @click="$router.push('/') && resetState">Назад</b-button>
        ...
        <div class="buttons">
          ...
          <download-excel v-if="outEcpData.length" :data="outEcpData" class="dlxls">
            <b-button id="download" class="btn-success download" >

              Скачать отчет
            </b-button>
            <b-tooltip  show target="download">Нажмите для скачивания отчета</b-tooltip>
          </download-excel>
        </div>
      </div>
